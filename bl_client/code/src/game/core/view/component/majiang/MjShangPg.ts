...
module majiang {
	export class MjShangPg extends eui.Component {
		private times;//那堆牌。
		private type;
		public color;
		private pbg;
		...
		public createChildren() {
			...
			this.topRectGroup.visible = false;
			switch (this.times) {
				case 1:
		...
		public showType(type, time) {
			switch (this.type) {
				...
				case 2:
				case 4:
					this.p_1.source = "top_angang_" + time + "_png";
					this.p_2.source = "top_angang_" + (time + 1) + "_png";
					this.p_3.source = "top_angang_" + (time + 2) + "_png";
					this.showColor_a();
					break;
			}
		}
		...
		public showRect(value) {
			if (value == 0) {
				...
				this.topRectGroup.visible = true;
			}
		}
