module niuniu {
	export class NiuniuSGameScene extends game.BaseComponent {
		//头像组 1 - 4
		...
		private btnzuo: eui.Button;
		private btnxia: eui.Button;
		...
		public async createChildren() {
			...
			// let header = this['header' + 1] as NiuniuNewHeader;
			// header.showWinPng(1);
			// return;
		...
		private showStartAni() {
			...
			startImage.scaleX = 1.2;
			startImage.scaleY = 1.2;
			startImage.verticalCenter = 0;
		...
		public async onTouchTap(e: egret.TouchEvent) {
			...
			switch (e.target) {
				...
				case this.restartBtn:
					...
					break;
				case this.btnzuo://菜单
					this.btnzuo.visible = false;
					this.btnxia.visible = true
					//此动画是只播放一次
					this.gnGroup.visible = true;
					this.rects.visible = true;
					this.gnGroup.mask = this.rects;//定义遮罩。
					var tw = egret.Tween.get(this.gnGroup);
					tw.to({ top: 91 }, 200);
					break;
				case this.btnxia://向上按钮
		...
		*
		...
		private showGold2Header(index1, index2) {
			...
			async.eachSeries(arr, (num, callback) => {
				...
				egret.Tween.get(image).wait(15 * (num / 2)).call(() => {
					...
					verticalCenter: header2.verticalCenter - 50 + _.random(-20, 20)// + header2.height / 2
		...
		private showNiu(pt, direction) {
			...
			switch (dir) {
				case "1":
					this.nnFenGroup.horizontalCenter = 25;
					this.nnFenGroup.verticalCenter = 310;
					break;
				case "2":
					this.nnFenGroup.horizontalCenter = 265;
					this.nnFenGroup.verticalCenter = 0
						;
					break;
				case "3":
					this.nnFenGroup.horizontalCenter = 15;
					this.nnFenGroup.verticalCenter = -270;
					break;
				case "4":
					this.nnFenGroup.horizontalCenter = -235;
					this.nnFenGroup.verticalCenter = 0;
					break;
			}
			this.effectGroup.addChild(this.nnFenGroup);
		}
